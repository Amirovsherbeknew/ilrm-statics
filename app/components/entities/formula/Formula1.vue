  ...
  <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6">
    ...
    <div class="flex items-center gap-1 mb-1">
      ...
      <h3 class="text-blue-900 font-semibold text-base">
        Калькулятор NPV — Банковский кредит
      </h3>
    </div>
    <p class="text-sm text-gray-400 mb-5">
      r рассчитывается автоматически: ставка кредита + риск проекта
    </p>
    ...
    <el-form :model="form" label-position="top" class="npv-form">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
        <el-form-item label="Ставка кредита (%)">
          <el-input v-model.number="form.rate" type="number" placeholder="18" />
          <span class="text-xs text-gray-400 mt-1">Например: 18</span>
        </el-form-item>
    ...
    <div
      v-if="result"
      class="mt-5 rounded-lg border p-4 max-h-[500px] overflow-y-auto"
      :class="result.npv >= 0
    ...
    >
      ...
      <pre class="text-sm text-gray-700 whitespace-pre-wrap font-mono leading-6">r = {{ form.rate }}% (кредит) + {{ form.risk }}% (риск) = {{ result.r }}%
...
NFV  = {{ result.sumPv.toFixed(2) }} - {{ form.investment.toFixed(2) }} = {{ result.npv.toFixed(2) }} млн</pre>
      ...
      <p class="text-sm text-gray-600 mt-3">
        При ставке кредита {{ form.rate }}% и риске {{ form.risk }}% проект приносит
        {{ result.npv >= 0 ? 'реальную прибыль' : 'убыток' }}
        {{ Math.abs(result.npv).toFixed(2) }} млн сум.
        {{ result.npv >= 0 ? 'Кредит брать выгодно.' : 'Кредит брать невыгодно.' }}
      </p>
    </div>
  </div>
</template>
...
function calculate() {
  const r = (form.rate || 0) + (form.risk || 0)
